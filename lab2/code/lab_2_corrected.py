...
plt.axvline(He_B, color='b', label="He-B")
plt.axvline(Ly_B, color='r', label="Ly-B")
...
fwhm_df = pd.DataFrame({
    "A": [popt[0], popt[3], popt[6]],
    "A_cov": [pcov[0][0], pcov[3][3], pcov[6][6]],
...
def get_new_C(delta_x):
    return delta_x / 2*(np.sqrt(2) * (np.log(2)**(1/2)))

fwhm_df["new_C"] = get_new_C(fwhm_df["S_FWHM"])

def get_intensity(A, A_error, C, C_error):
    intensity = 2 * np.pi * A * C
    intensity_error = intensity * np.sqrt(((A_error/A)**2) +((C_error/C)**2)) 
    return intensity, intensity_error

fwhm_df["intensity"], fwhm_df["intensity_error"] = get_intensity(fwhm_df["A"], np.sqrt(fwhm_df["A_cov"]), fwhm_df["new_C"], np.sqrt(fwhm_df["C_cov"]))

print (fwhm_df)

for idx, row in fwhm_df.iterrows():
    
    print ("The value of FWHM is: {} +- {}".format(row["S_FWHM"], np.sqrt(row["S_FWHM_error"])))
    print ("The value of intensity is: {} +- {}".format(row["intensity"], np.sqrt(row["intensity_error"])))
